...
export function ShellProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshHistory = () => setHistoryKey((prev) => prev + 1);
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100 font-sans overflow-hidden">
      ...
      <aside
        ...
        } transition-all duration-300 ease-in-out bg-slate-900 border-r border-slate-800 flex flex-col z-40 relative select-none`}
      >
        {/* Sidebar Header */}
        ...
        {/* Nav Links */}
        <nav className="flex-1 px-3 py-2 space-y-1 font-mono text-xs">
          <button
            onClick={() => setLocation("/")}
        ...
        {/* Footer */}
        <div className="p-3 border-t border-slate-800 bg-slate-950/40 text-[10px] font-mono text-slate-500 flex justify-between items-center">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-slate-950">
        ...
        <header className="h-10 border-b border-slate-800 bg-slate-900/80 px-3 flex items-center gap-3 z-30 font-mono text-xs">
          {!isSidebarOpen && (
            <button
              onClick={toggleSidebar}
              className="p-1.5 hover:bg-slate-800 text-amber-400 hover:text-amber-300 rounded transition-colors"
